...
const AccountsReceivablePage = () => {
  ...
  const filteredReceivables = receivables.filter(receivable => {
    const matchesSearch = receivable.customer.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         ...
                         (receivable.products && receivable.products.some(product => 
  ...
  const handleExport = () => {
    const csvData = [
      ['Customer', 'Products', 'Total', 'Paid', 'Balance', 'Transaction Date', 'Due Date', 'Status'],
      ...filteredReceivables.map(r => [
        r.customer,
        r.products && r.products.length > 0 
          ? r.products.map(p => `${p.name}${p.quantity > 1 ? ` ×${p.quantity}` : ''}`).join('; ')
          : 'No products',
        r.total,
        r.paid,
        r.balance,
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm border mb-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between space-y-4 md:space-y-0">
            <div className="relative flex-1 max-w-md">
              ...
              <input
                type="text"
                placeholder="Search customer, sale #, or product..."
                value={searchTerm}
                  ...
                  <tr key={receivable.saleId} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="p-4">
                      <div className="text-sm">
                        {receivable.products && receivable.products.length > 0 ? (
                          <div className="space-y-1">
                            {receivable.products.slice(0, 2).map((product, index) => (
                              <div key={index} className="text-gray-700">
                                <span className="font-medium">{product.name}</span>
                                {product.quantity > 1 && <span className="text-gray-500"> ×{product.quantity}</span>}
                              </div>
                            ))}
                            {receivable.products.length > 2 && (
                              <div className="text-gray-500 text-xs">
                                +{receivable.products.length - 2} more
                              </div>
                            )}
                          </div>
                        ) : (
