...
const Vendor = () => {
  ...
  const filteredPurchaseRecords = purchaseRecords.filter(purchase =>
    purchase.vendorName?.toLowerCase().includes(purchaseSearchTerm.toLowerCase()) ||
    purchase.productName?.toLowerCase().includes(purchaseSearchTerm.toLowerCase()) ||
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          {[
            ...
            { label: 'Total Purchases', value: activeTab === 'records' ? filteredPurchaseRecords.length : purchaseRecords.length, icon: ShoppingCart, bgColor: 'bg-green-50', textColor: 'text-green-600' },
            { label: 'Total Amount', value: formatCurrency((activeTab === 'records' ? filteredPurchaseRecords : purchaseRecords).reduce((sum, p) => sum + (p.amountDue || 0), 0)), icon: DollarSign, bgColor: 'bg-purple-50', textColor: 'text-purple-600' },
            { label: 'Outstanding Balance', value: formatCurrency((activeTab === 'records' ? filteredPurchaseRecords : purchaseRecords).reduce((sum, p) => sum + ((p.amountDue || 0) - (p.amountPaid || 0)), 0)), icon: AlertCircle, bgColor: 'bg-red-50', textColor: 'text-red-600' }
          ].map((stat, index) => {
        ...
        {activeTab === 'vendors' ? (
          ...
          <div className="space-y-6">
            {/* Search Bar for Purchase Records */}
            <div className="relative max-w-md">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 w-5 h-5" />
              <input
                type="text"
                placeholder="Search purchases by vendor, product, or ID..."
                value={purchaseSearchTerm}
                onChange={(e) => setPurchaseSearchTerm(e.target.value)}
                onClick={(e) => e.stopPropagation()}
                className="w-full pl-12 pr-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-4 focus:ring-green-500/20 focus:border-green-500 outline-none transition-all"
              />
            </div>
            
            {/* Payment Dashboard */}
            <PaymentMethodDashboard purchaseRecords={activeTab === 'records' ? filteredPurchaseRecords : purchaseRecords} />
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filteredPurchaseRecords.map((purchase, index) => {
                      const balance = (purchase.amountDue || 0) - (purchase.amountPaid || 0);
